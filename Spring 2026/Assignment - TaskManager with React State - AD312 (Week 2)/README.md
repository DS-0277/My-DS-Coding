**Assignment - TaskManager with React State - AD312 (Week 2)**


**1. Objective:** Learn to update objects in React state using the spread syntax in a non-form context by building a simple task manager application. There are 2 objectives: (1) Create a simple task manager where users can add new tasks and mark tasks as completed; (2) Demonstrate the correct way to update arrays and objects within the React state. Note: While this assignment can be completed using Vite or Expo, the instructions below are specifically tailored for Vite.  
